...
export class Store {
  ...
  public async addLink(url: string): Promise<string> {
    const inserted = await this._db.insert(links).values({
      url: url,
      createdAt: new Date()
  ...
  public async getLink(id: string): Promise<string | undefined> {
    try {
      ...
      const [link] = await this._db.select().from(links).where(eq(links.id, numericId)).limit(1)

      if (!link) return undefined

      await this._db.update(links).set({
        used: link.used + 1
  ...
  public async list(): Promise<(Omit<Link, 'id'> & { id: string})[]> {
    const list = await this._db.select().from(links).orderBy(desc(links.createdAt))
